...
class Database:
    ...
    def get_block_users(self):
        self.cursor.execute(
            "SELECT block_users.id, IIF(block_users.username IS NOT NULL, '@' || block_users.username, block_users.username), users.name, users.id, users.chat_id, users.register_datetime FROM users RIGHT JOIN block_users ON users.id = block_users.user_id ORDER BY block_users.id")
        result = self.cursor.fetchall()
        return result
